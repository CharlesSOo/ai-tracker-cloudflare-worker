...
GET and HEAD requests with known crawler/bot user-agent hints are sent to `/api/ingest`. **All matching crawler paths count**, including pages, `robots.txt`, sitemaps, `llms.txt`, scripts, styles and images; there is no static-asset exclusion. The tracker determines final bot classification. User-agent hints can be spoofed and are not proof of bot identity.
